...
// Enhanced createPlanetLayers function with advanced clipping techniques
function createPlanetLayers(planetObject) {
	...
	if (planetObject.layerVisible) {
		...
		// Define clipping plane - cutting the planet in half
		const clipPlane = new THREE.Plane(new THREE.Vector3(1, 0, 0), 0);

		// Ensure clipping is enabled in the renderer
		renderer.localClippingEnabled = true;

		// Get layer data
		const layersData = planetObject.data.layers;

		// Spacing constants for the exploded view
		const explodeOffset = planetObject.data.radius * 0.8; // Space between intact and exploded views
		const layerSpacing = planetObject.data.radius * 0.2; // Space between internal layers in exploded view

		// Create the intact hemisphere (right side)
		const intactGroup = new THREE.Group();
		layersGroup.add(intactGroup);

		// Create stencil group for the right hemisphere (intact view)
		const stencilGroup = createHemisphereStencilGroup(
			planetObject.data.radius,
			clipPlane,
			1  // render order
		);
		intactGroup.add(stencilGroup);

		// Create each layer of the planet with proper clipping
		for (let i = 0; i < layersData.length; i++) {
			const layer = layersData[i];
			const layerRadius = planetObject.data.radius * layer.radius;

			// Create sphere geometry for this layer
			const sphereGeometry = new THREE.SphereGeometry(layerRadius, 32, 32);
			...
			const sphereMaterial = new THREE.MeshStandardMaterial({
				...
				stencilWrite: i === layersData.length - 1 ? true : false,
				stencilRef: 0,
			...
			// Apply texture to outermost layer if it's Earth
			if (i === layersData.length - 1 && planetObject.data.name === "Earth") {
				try {
					...
					console.log("Couldn't load texture, using color instead");
				}
			}

			// Create the mesh and add to the intact group
			const sphereMesh = new THREE.Mesh(sphereGeometry, sphereMaterial);
			sphereMesh.renderOrder = 2; // Render after stencil
			intactGroup.add(sphereMesh);
		}

		// Now create the exploded view (left side)
		const explodedGroup = new THREE.Group();
		explodedGroup.position.x = -explodeOffset; // Position to the left
		layersGroup.add(explodedGroup);

		// Create exploded view with each layer separated - using clipping plane in opposite direction
		const explodedClipPlane = new THREE.Plane(new THREE.Vector3(-1, 0, 0), 0);

		// Create stencil group for the left hemisphere (exploded view)
		const explodedStencil = createHemisphereStencilGroup(
			planetObject.data.radius,
			explodedClipPlane,
			3  // render order
		);
		explodedGroup.add(explodedStencil);

		// Create each layer with spacing for exploded view
		for (let i = 0; i < layersData.length; i++) {
			const layer = layersData[i];
			const layerRadius = planetObject.data.radius * layer.radius;

			// Calculate layer position - innermost layers move further left
			const layerOffset = (layersData.length - i - 1) * layerSpacing;

			// Create sphere geometry
			const sphereGeometry = new THREE.SphereGeometry(layerRadius, 32, 32);

			// Create material with clipping plane for hemisphere
			const sphereMaterial = new THREE.MeshStandardMaterial({
				color: layer.color,
				metalness: 0.2,
				roughness: 0.7,
				clippingPlanes: [explodedClipPlane],
				side: THREE.DoubleSide,
				// Stencil settings
				stencilWrite: true,
				stencilRef: 0,
			...
			// Create the hemisphere mesh
			const sphereMesh = new THREE.Mesh(sphereGeometry, sphereMaterial);
			sphereMesh.renderOrder = 4;
			sphereMesh.position.x = -layerOffset; // Offset more for inner layers
			explodedGroup.add(sphereMesh);

			// Add layer label
			addLayerLabel(explodedGroup, layer.name, layerRadius, layerOffset, i);
		}

		// Rotate the entire group for better view
		layersGroup.rotation.y = Math.PI / 2;
...
// Helper function to create a stencil group for hemisphere rendering
function createHemisphereStencilGroup(radius, plane, renderOrder) {
	const group = new THREE.Group();
	const sphereGeometry = new THREE.SphereGeometry(radius * 1.1, 32, 32);

	// Base stencil material
	...
	const backMesh = new THREE.Mesh(sphereGeometry, backMat);
	...
	const frontMesh = new THREE.Mesh(sphereGeometry, frontMat);
...
function addLayerLabel(parent, layerName, layerRadius, offset, index) {
	...
	// Position the label
	const labelOffset = layerRadius * 1.2;
	sprite.position.set(
		-offset - labelOffset,      // X position (further left of layer)
		(layersData.length - index) * 1.0 - layerRadius * 0.5,  // Y position (stacked vertically)
		0
	);
	...
	const lineGeometry = new THREE.BufferGeometry().setFromPoints([
		new THREE.Vector3(-offset - layerRadius * 0.5, 0, 0),
		new THREE.Vector3(-offset - labelOffset + 1, sprite.position.y, 0)
	]);
...
// Modified onMouseClick function to improve camera positioning for layer view
function onMouseClick(event) {
	...
	if (intersects.length > 0) {
		...
		if (clickedPlanet) {
			...
			// Calculate proper camera positioning to view the cross-section
			// Position camera slightly above and to the side for better view of layers
			cameraOffset = new THREE.Vector3(
				selectedPlanet.data.radius * 2,
				selectedPlanet.data.radius * 1.5,
				selectedPlanet.data.radius * 3
			);
